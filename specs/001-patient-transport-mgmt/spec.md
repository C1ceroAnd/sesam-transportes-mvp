...
### História de Usuário 1 - Cadastrar e Agendar Paciente em Viagem (Prioridade: P1)
...
**Por que esta prioridade**: É o fluxo central do sistema — sem ele nenhuma outra funcionalidade faz sentido. Representa o ciclo completo de atendimento a um paciente.

**Teste Independente**: Pode ser testado de forma isolada criando um paciente, uma viagem e realizando o agendamento — verifica cadastro, vinculação de acompanhante, validação de capacidade e registro do destino.

**Cenários de Aceite**:

1. **Dado que** o administrativo está na tela de cadastro de paciente, **Quando** preenche nome, CPF válido, telefone e seleciona um ponto de embarque da lista fixa, **Então** o paciente é salvo e aparece na listagem de pacientes.
2. **Dado que** um paciente cadastrado e uma viagem com vagas disponíveis, **Quando** o administrativo agenda o paciente informando motivo do deslocamento, ponto de desembarque e destino real da consulta, **Então** o agendamento é criado e a contagem de vagas da viagem é atualizada.
3. **Dado que** uma viagem com 28 vagas preenchidas, **Quando** o administrativo tenta agendar mais um paciente, **Então** o sistema impede o agendamento e exibe mensagem de capacidade máxima atingida.
4. **Dado que** um paciente sendo agendado com um acompanhante (que ocupa vaga), **Quando** o agendamento é confirmado, **Então** o sistema conta 2 vagas utilizadas (paciente + acompanhante) e valida a capacidade total.
5. **Dado que** um paciente com prioridade Alta (oncológico), **Quando** o paciente é listado em uma viagem, **Então** sua prioridade é visível no agendamento.

---

### História de Usuário 2 - Configurar Viagem (Motorista e Veículo) (Prioridade: P1)
...
**Por que esta prioridade**: A viagem é o objeto central ao qual todos os agendamentos se vinculam. Sem a viagem configurada, não é possível agendar pacientes.

**Teste Independente**: Pode ser testado criando uma viagem, selecionando motorista e cadastrando placa/modelo do veículo — verifica criação da viagem e associação dos dados operacionais.

**Cenários de Aceite**:

1. **Dado que** o administrativo vai criar uma nova viagem, **Quando** informa a data e seleciona um motorista da lista disponível, **Então** a viagem é criada com status "Aguardando" e o motorista fica registrado.
2. **Dado que** uma viagem criada, **Quando** o administrativo registra placa e modelo do veículo, **Então** os dados do veículo ficam salvos e associados à viagem.
3. **Dado que** a lista de motoristas disponíveis, **Quando** o administrativo abre a tela de seleção de motorista, **Então** o sistema indica qual motorista está na vez na escala rotativa (baseado na viagem anterior).
4. **Dado que** duas viagens consecutivas, **Quando** o administrativo verifica a sugestão de escala, **Então** motoristas alternam entre as viagens (Motorista A → Motorista B → Motorista A...).

---

### História de Usuário 3 - Registrar Presença e Status de Retorno (Prioridade: P2)
...
**Por que esta prioridade**: Fundamental para o controle operacional, mas depende das User Stories 1 e 2 estarem funcionando.

**Teste Independente**: Pode ser testado em uma viagem com agendamentos existentes — marca presença/falta e registra retorno dos pacientes.

**Cenários de Aceite**:

1. **Dado que** uma viagem com pacientes agendados, **Quando** o administrativo abre a lista de passageiros e marca presença de um paciente, **Então** o status do paciente na viagem muda para "Presente".
2. **Dado que** uma viagem com pacientes agendados, **Quando** o administrativo marca falta de um paciente, **Então** o status muda para "Faltou".
3. **Dado que** um paciente marcado como presente em uma viagem, **Quando** o administrativo registra o status de retorno como "Voltou no dia", **Então** o registro de retorno é salvo e o histórico do paciente é atualizado.
4. **Dado que** um paciente que ficou em Teresina, **Quando** o administrativo registra "Ficou em Teresina", **Então** o sistema salva esse status e o paciente aparece como pendente de retorno.
5. **Dado que** um paciente que voltou por conta própria, **Quando** o administrativo registra "Voltou por conta própria", **Então** o status é salvo com essa descrição.

---

### História de Usuário 4 - Cancelar Agendamento (Prioridade: P2)
...
**Por que esta prioridade**: Operacionalmente necessário, mas é uma função secundária ao fluxo de agendamento.

**Teste Independente**: Pode ser testado cancelando um agendamento existente e verificando que a contagem de vagas volta ao estado anterior.

**Cenários de Aceite**:

1. **Dado que** um paciente agendado em uma viagem, **Quando** o administrativo cancela o agendamento, **Então** o agendamento é removido e a(s) vaga(s) liberada(s) reflete na contagem da viagem.
2. **Dado que** um paciente agendado com acompanhante, **Quando** o administrativo cancela o agendamento, **Então** 2 vagas são liberadas (paciente + acompanhante).
3. **Dado que** uma viagem que já foi realizada (passada), **Quando** o administrativo tenta cancelar um agendamento, **Então** o sistema impede o cancelamento e exibe mensagem explicativa.

---

### História de Usuário 5 - Consultar Viagens e Gerar Relatório PDF (Prioridade: P3)
...
**Por que esta prioridade**: Funcionalidade de consulta e relatório; agrega valor mas não bloqueia operação diária.

**Teste Independente**: Pode ser testado listando viagens por uma data e gerando PDF do histórico de um paciente — verifica filtragem e exportação.

**Cenários de Aceite**:

1. **Dado que** viagens cadastradas em datas diferentes, **Quando** o administrativo filtra por uma data específica, **Então** apenas as viagens daquela data são exibidas com seus passageiros e status.
2. **Dado que** a listagem de uma viagem, **Quando** o administrativo visualiza os passageiros, **Então** cada passageiro exibe nome, ponto de embarque, prioridade e status de presença/retorno.
3. **Dado que** um paciente com histórico de viagens, **Quando** o administrativo solicita o relatório PDF, **Então** um PDF é gerado contendo todas as viagens do paciente com datas, motivos, pontos de desembarque e status de retorno.
4. **Dado que** o relatório PDF gerado, **Quando** o administrativo abre o arquivo, **Então** o relatório é legível, ordenado por data e contém todos os campos relevantes.
